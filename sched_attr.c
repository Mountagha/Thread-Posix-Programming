...
void *thread_routine(void *arg){
     ...
     * so just report that thread ran, and exit.
     */

#if defined(_POSIX_THREAD_PRIORITY_SCHEDULING) && !defined (sun)
    status = pthread_getschedparam(
        pthread_self(), &my_policiy, &my_param
    );
    if (status != 0)
        err_abort(status, "Get sched");
    printf("Thread_routine running at %s/%d\n", 
    (my_policiy == SCHED_FIFO ? "FIFO"
        : (my_policiy == SCHED_RR ? "RR"
        : (my_policiy == SCHED_OTHER ? "OTHER"
        : "UNKNOWN"))),
...
int main(int argc, char *argv[]){
     ...
     * scheduling parameters. Note that it is particulary important
     * that you remember to set the inheritsched attribute to 
     * PTHREAD_EXPLICIT_SCHED, or the policy and the priority that you've
    ...
    if (status != 0)
         ...
         * middle of the priority range allowed for SCHED_RR. This should
         * ensure that the thread will be run, without blocking everything
         * else. Because any assumptions about how a thread's priority 
         * interacts with other threads (even in other processors) are 
         * nonportable, especially on an implementation that defaults to 
         * system contention scope, you may have to adjust this code before
         * it will work on some systems.
         */
        ...
        thread_param.sched_priority = (rr_min_priority + rr_min_priority)/2;
