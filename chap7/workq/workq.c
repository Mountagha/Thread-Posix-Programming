...
int workq_destroy (workq_t *wq) {
    ...
    if (wq->counter > 0){
         ...
         * using the "cv" condition for 2 separate predicates
         ...
         * wait and be signaled exactly one
         */
...
static void *workq_server(void *arg) {
    ...
    while (1) {
        ...
        while (wq->first == NULL && !wq->quit) {
            // Server threads time out after spending 2 seconds waiting for new work,
            // and exit.
            ...
            if (status == ETIMEDOUT) {
                ...
                /*
                 * This shouldn't happen, so the work queue package 
                 * should fail. Because the work queue API is asynchronous, 
                 * that would add complications. Because the chance of
                 * failure is slim, I choose to avoid that complication.
                 * The server thread will return, and allow another server
                 * thread to pick up the work later. Note that if this 
                 * were the only server thread, the queue wouldn't be
                 * serviced until a new work item is queued. That could
                 * be fixed by creating a new server here.
                 */
