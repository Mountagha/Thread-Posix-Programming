...
/* 
 * The "alarm" structure now contains the time_t (time since the epochs in seconds)
 * for each alarm, so that they can be sorted. Storing the requested number of seconds
 * would not be enough, since the "alarm thread" cannot tell how long it has been on the list.
 *
 */
...
void alarm_insert(alarm_t *alarm){
	 ...
	 * ("next" is NULL, and "last" points to the link field of the last item,
	 * or the list header.)
	 */
	if (next == NULL){
		*last = alarm;
...
#endif
	/*
	 * Wake the alarm thread if it is not busy (that is, if current_alarm is 0, 
	 * signifying that it's waiting for work), or if the new alarm comes before the one
 ...
 */
 ...
 void *alarm_thread(void *arg) {
	 ...
	 * disintegrated when the process exits. Lock the mutex at the 
	 * start -- it will be unlock during condition waits, so the 
	 * main thread can insert alarms.
	 */
