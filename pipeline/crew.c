 ...
 * crew_start, and each worker maye queu additional items.
 */
 ...
 * crew. It contains the "identify" of each worker.
 */
 ...
 * crew synchronization stage and staging area.
 */
...
typedef struct crew_tag {
    ...
    work_t  *first, *last;           // First and last work item
...
/*
 * The thread start routine for crew threads. Wait until "go"
...
void *worker_routine (void *arg){
    ...
    // There won't be any work when the crew is created, so
    // wait until something's put on the queue.
    ...
    while (1) {
        ...
        if (S_ISLNK (filestat.st_mode))
            ...
            // all the files onto the queue as new work items.
            ...
            while (1) {
                ...
                strcat(new_work->path, result->d_name);
                new_work->next = NULL;
                status = pthread_mutex_lock(&crew->mutex);
                if (status != 0)
         ...
         * waiters (trying to collect result or start a new calculation) 
...
/*
 * Pass a file to a work crew previously created
 * using crew_create
 */
