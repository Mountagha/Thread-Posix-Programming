...
/* 
 * Internal structure describing a "stage" in the pipeline.
 * One for each thread, plus a "result stage" where the final 
 * thread can stash the value.
 * 
 */
...
/* 
 * internal function to send a "message" to the specified pipe stage. 
 * Threads use this to pass along the modified data item.
 */
 ...
 * data is initialized and the threads. They'll 
 * wait for data.
 */
...
int pipe_create (pipe_t *pipe, int stages){
    ...
    *link = (stage_t *)NULL; // Terminate the list
    ...
    /* 
     * Create the threads for the pipe stages onl after all
     * the data is initialized ( including all links ). Note 
     * that the last stage doesn't get a thread, it's just a 
     * receptacle for the final pipeline value.
     * 
 ...
 * (note taht the pipe will stall when each stage fills, 
 * until the result is collect)
 */
...
int pipe_result(pipe_t *pipe, long *result){
    ...
    if (status != 0)
        err_abort(status, "Lock tail pipe");
...
int main(int argc, char *argv[]){
    ...
    printf("Enter integer values, or \"=\" for the next result\n");
